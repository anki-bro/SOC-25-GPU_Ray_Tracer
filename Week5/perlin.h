...
public:
    perlin() {
        for (int i = 0; i < point_count; ++i) {
            randomvector[i] = Vec3::random_unit_vector();
        }
        ...
        for (int di = 0; di < 2; ++di) {
            for (int dj = 0; dj < 2; ++dj) {
                for (int dk = 0; dk < 2; ++dk) {
                    c[di][dj][dk] = randomvector[perm_x[(i + di) & 255] ^ perm_y[(j + dj) & 255] ^ perm_z[(k + dk) & 255]];
                }
            }
        }
    ...
    double turbulence(const point3& p, int depth = 1) const {
        auto accum = 0.0;
        auto temp_p = p;
        auto weight = 1.0;
